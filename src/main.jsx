...
import { Activity, Disc3, Radio, RefreshCw, Search } from 'lucide-react'
import './styles.css'
...
function Header({ view, setView, lidarr }) {
  return (
...
function ActivityView({ lidarr }) {
  ...
  return (
    <section>
      <div className="page-heading">
        <div><p>02 / LIDARR ACQUISITION</p><h1>Activity</h1></div>
...
function App() {
  ...
  return (
    <div className="app-shell">
      <Header view={view} setView={setView} lidarr={lidarr} />
      <main>{view === 'catalog' ? <CatalogView lidarr={lidarr} /> : <ActivityView lidarr={lidarr} />}</main>
    </div>
  )
}
