...
export class SlskdAdapter implements ServiceAdapter {
  ...
  async #request(path: string, method: string, context: OperationContext, body?: unknown): Promise<unknown> { const signal = context.signal ? AbortSignal.any([context.signal, AbortSignal.timeout(this.#timeout)]) : AbortSignal.timeout(this.#timeout); let response: Response; try { response = await this.#fetch(new URL(path, this.#base), { method, signal, headers: { Accept: 'application/json', 'Content-Type': 'application/json', 'X-API-Key': this.#key, 'X-Arcadia-Operation-Id': context.operationId }, ...(body === undefined ? {} : { body: JSON.stringify(body) }) }) } catch (cause) { throw this.#error('unavailable', signal.aborted ? 'slskd request timed out or was cancelled' : 'slskd is unavailable', true, undefined, cause) } if (!response.ok) throw this.#error(response.status === 401 || response.status === 403 ? 'authentication' : response.status === 404 ? 'not-found' : response.status === 429 ? 'rate-limited' : response.status >= 500 ? 'transient-provider-failure' : 'invalid-request', `slskd request failed with status ${response.status}`, response.status === 429 || response.status >= 500, response.status); if (response.status === 204) return undefined; try { return await response.json() } catch (cause) { throw this.#error('transient-provider-failure', 'slskd returned invalid JSON', true, response.status, cause) } }
