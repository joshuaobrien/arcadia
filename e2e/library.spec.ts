...
test('browses the library and opens an album', async ({ page }) => {
  ...
  await expect(page.getByRole('button', { name: 'Open Static Bloom by Needle Test Ensemble' })).toBeVisible()

  await page.getByRole('button', { name: 'Open Static Bloom by Needle Test Ensemble' }).click()
